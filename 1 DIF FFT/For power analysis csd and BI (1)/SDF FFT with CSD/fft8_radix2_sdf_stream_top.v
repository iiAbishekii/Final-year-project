...
// Description : 8-point Radix-2 SDF DIF FFT - Top Level
//
// Stage 1 : delay=4, W^0 only (no multiply)
// Stage 2 : delay=2, W^0 and W^2=-j (no multiply)
// Stage 3 : delay=1, W^0..W^3  ? W^1,W^3 via CSD multiplier
//
// Top level is IDENTICAL to original - CSD is fully contained
// inside stage3_radix2_sdf_stream and csd_sqrt2_inv.
// No port changes anywhere.
// ============================================================
module fft8_radix2_sdf_stream_top (
    input  wire        clk,
    input  wire        valid_in,
    ...
    // Stage-1 ? Stage-2 gating (skip first 4 warmup outputs)
    ...
    // Stage-2 ? Stage-3 gating (skip first 2 warmup outputs)
    always @(posedge clk) begin
        s23_valid <= 0;
        if (s2_valid) begin
    ...
    // ----------------------------------------------------------
    // STAGE 3  (CSD twiddle inside - no port change)
    // ----------------------------------------------------------
