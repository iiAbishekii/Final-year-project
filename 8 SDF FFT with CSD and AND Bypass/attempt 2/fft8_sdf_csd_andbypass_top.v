module fft8_sdf_csd_andbypass_top (
    ...
    // ── Input register — clock-gated by valid_in ─────────────
    ...
    always @(posedge clk) begin
        r1_valid <= valid_in;
        if (valid_in) begin       // ICG-eligible enable
            r1_re <= xin_re;
            r1_im <= xin_im;
        end
    end
    ...
    // ── Pipeline register Stage1→Stage2 — clock-gated ────────
    ...
    always @(posedge clk) begin
        s12_valid <= s1_valid;
        if (s1_valid) begin       // ICG-eligible enable
            s12_re <= s1_re;
            s12_im <= s1_im;
        end
    end
    ...
    // ── Pipeline register Stage2→Stage3 — clock-gated ────────
    ...
    always @(posedge clk) begin
        s23_valid <= s2_valid;
        if (s2_valid) begin       // ICG-eligible enable
            s23_re <= s2_re;
            s23_im <= s2_im;
        end
    end
